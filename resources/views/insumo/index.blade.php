...
@section('title', 'Consultar insumos')

@section('content_header')
    <h1 style="align-content: center">Consultar insumos</h1>
@stop
...
@section('content')
    <table id="table1" class="table table-striped shadow-lg mt-4">
        ...
        <tbody>
            ...
            <tr>
                <td>{{$insumo->id}}</td>
                <td>{{$insumo->tipo_insumo ?
                    $insumo->tipo_insumo->nombre:""}}</td>
                <td>{{$insumo->codigo}}</td>
                <td>{{$insumo->nombre}}</td>
                <td>@if ( $insumo->es_reactivo == 1) Reactivo
                    @else No Reactivo
                    @endif</td>
                 ...
                 <td>
                    <div class="form-group row">
                        <div class="col-sm-6">
                    <a href="{{ route('insumo.edit', $insumo->id)}}" class="btn btn-primary">Editar</a>
                    </div>
                    <div class="col-sm-6">
